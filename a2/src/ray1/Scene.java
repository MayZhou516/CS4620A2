...
public class Scene {
	...
	*/
	...
	private boolean intersect(IntersectionRecord outRecord, Ray rayIn, boolean anyIntersection) {
		...
		IntersectionRecord testRec = new IntersectionRecord();
		for(Surface i: this.getSurfaces()){
			if(i.intersect(testRec, rayIn)){
				if(testRec.t < outRecord.t && anyIntersection){
					return true;
				}
				else if(testRec.t < min_t){
					min_t = testRec.t;
					outRecord.set(testRec);
					returnedValue = true;
					outRecord.surface = i;
				}
			}
		...
		//if the new outRecord value of t is smaller than the original t, then return trues
		//if(outRecord.t < oT){return true;}
		//else{return false;}
	}
}