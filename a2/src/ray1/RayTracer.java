...
public class RayTracer {
	...
	public static void shadeRay(Colorf outColor, Scene scene, Ray ray) {
		...
		// scene is a bunch of surface, has geometry and has a shader, shader calculates color
		// call on shader through, look at rbgnormal
		IntersectionRecord i = new IntersectionRecord();
		if(!scene.getFirstIntersection(i, ray)){
			outColor.set(scene.getBackColor());
			return;
		}
		Shader s = i.surface.getShader();
		s.shade(outColor, scene, ray, i);
	}
}
